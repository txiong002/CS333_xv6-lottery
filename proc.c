...
//# error LOTTERY code.
  ...
  for(;;){
    ...
    total_tickets = sum_of_nice();
    if(total_tickets > 0){

        golden_ticket = (rand() % total_tickets) + 1;
        total_tickets = 0;
        for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
            if(p->state != RUNNABLE)
                continue;

            total_tickets += p->nice_value;
...
sum_of_nice()
{
  ...
  //loop through runnable processes
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
      if(p->state == RUNNABLE) {
...
int
renice(int update_nice, int pid)
{
    int i;
    //if new nice value is within range
    if(MIN_NICE_VALUE <= update_nice && update_nice <= MAX_NICE_VALUE){
        acquire(&ptable.lock);
        //loop through process to find pid
        for(i = 0; i < NPROC; i++){
            //if pid exist
            if(ptable.proc[i].pid == pid){
                ptable.proc[i].nice_value = update_nice;
                //release(&ptable.lock);
                break;
            }
        }
        ...
        //cprintf("\nNice value: %d is out of bounds", update_nice);
        return 1;
    }
    release(&ptable.lock);
    return 0;
...
//# error this is an excellent place to add some new data to the o/p of cps
    struct rtcdate date = ptable.proc[i].begin_date;
            cprintf("\t%d",date.year);
            if(date.month < 10){
                cprintf("-0%d", date.month);
            }else {
                cprintf("-%d", date.month);
            }
            if(date.day < 10){
                cprintf("-0%d", date.day);
            }else {
                cprintf("-%d", date.day);
            }
            if(date.hour < 10){
                cprintf(" 0%d", date.hour);
            }else {
                cprintf(" %d", date.hour);
            }
            if(date.minute < 10){
                cprintf(":0%d", date.minute);
            }else {
                cprintf(":%d", date.minute);
            }
            if(date.second < 10){
                cprintf(":0%d", date.second);
            }else {
                cprintf(":%d", date.second);
            }
            cprintf("\t%d\t%d\t%d"
